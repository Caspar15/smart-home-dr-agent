...
# -- CNN-LSTM architecture (Conv1D x2 -> LSTM x2) -----------------------------
# Source: multi_household/forecasting/per_house_lstm.py::CNNLSTM
# Adaptation: head outputs horizon steps (original: 1-step).
# Validated on H20/H2: RMSE_scaled within 1% of BaseloadLSTM on
# baseload target. Chosen as joint-publication unified architecture.
_CNN_FILTERS = 64
_CNN_HIDDEN  = 128
_CNN_LAYERS  = 2
_CNN_DROPOUT = 0.25
...
class BaseloadCNNLSTM(nn.Module):
    ...
    FedAvg interface (identical to BaseloadLSTM):
        get_weights()        -> list[np.ndarray]
        set_weights(weights) -> None
    """

    def __init__(self, input_size: int = 5, conv_filters: int = _CNN_FILTERS,
                 hidden_size: int = _CNN_HIDDEN, num_layers: int = _CNN_LAYERS,
                 dropout: float = _CNN_DROPOUT, horizon: int = 36):
        super().__init__()
        ...
        lstm_drop  = dropout if num_layers > 1 else 0.0
        self.lstm  = nn.LSTM(conv_filters, hidden_size, num_layers,
                             batch_first=True, dropout=lstm_drop)
        self.head  = nn.Linear(hidden_size, horizon)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = h.permute(0, 2, 1)        # back to (B, L, conv_filters)
        _, (h_n, _) = self.lstm(h)
...
def build_model(arch: str, cfg: dict) -> nn.Module:
    """Instantiate the requested architecture. Raises ValueError on unknown arch."""
    if arch == "cnn_lstm":
        return BaseloadCNNLSTM(
    ...
    raise ValueError(f"Unknown arch '{arch}'. Choose 'lstm' or 'cnn_lstm'.")
...
# ── Per-house driver ──────────────────────────────────────────────────────────
def run_house(house_id: int, cfg: dict, device: str, arch: str = "lstm") -> dict:
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase 2 — per-house baseload forecast (LSTM or CNN-LSTM)"
    )
    parser.add_argument("--arch",     type=str,   default="lstm",
                        choices=["lstm", "cnn_lstm"],
                        help="Model architecture: lstm (default) or cnn_lstm")
    ...
    all_results = []
    for h in cfg["houses"]:
        result = run_house(h, cfg, device, arch=args.arch)
    ...
    print("  [6] OK  BaseloadLSTM / BaseloadCNNLSTM: get_weights() / set_weights()")
    print("          local_train() is arch-agnostic; shared by both model classes")
